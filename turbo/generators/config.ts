    ...
    prompts: [
      {
        type: 'input',
        name: 'name',
        message: 'What is the name of the package? (You can skip the `@pixelstack/` prefix)',
      },
      {
        type: 'input',
        name: 'deps',
    ...
    actions: [
      (answers) => {
        if ('name' in answers && typeof answers.name === 'string') {
          if (answers.name.startsWith('@pixelstack/')) {
            answers.name = answers.name.replace('@pixelstack/', '');
          }
        }
